...
public class StationListFragment extends Fragment implements IAssetsSubFragment {
    ...
    private void updateListView() {
        ...
        for (int x = 0; x < currentStationList.length; x++)
            stationIDs[x] = ((AssetsEntity.Station) currentStationList[x]).getLocationID();
    ...
    private void countSubAssets(ArrayList<AssetsEntity> assets, int rootStationID) {
        for (AssetsEntity entity : assets) {
            ...
            if (currentItemCount != null)
                itemCounts.get(rootStationID).put(typeID, currentItemCount + quantity);
