...
public class InventoryListFragment extends Fragment implements IAssetsSubFragment {
    ...
    private void updateView() {
        if (adapter == null) {
            ...
            if (parentFragment.getPrices() != null && parentFragment.getPrices().size() > 0)
        ...
        if (savedScrollPoint != null) {
            if (absListView instanceof ListView)
                ((ListView) absListView).setSelectionFromTop(savedScrollPoint[0], savedScrollPoint[1]);
            else if (absListView instanceof GridView)
                ((GridView) absListView).setSelection(savedScrollPoint[0]);
    ...
    private class InventoryArrayAdapter extends ArrayAdapter<AssetsEntity> {
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null)
                itemView = (LinearLayout) inflater.inflate(layoutResID, parent, false);
        ...
        private void setupAsset(final LinearLayout rootView, AssetsEntity.Item item) {
            ...
            if (typeNamesLoaded) {
                if (parentFragment.getTypeInfo().get(typeID) != null)
                    text.setText(parentFragment.getTypeInfo().get(typeID).typeName);
        ...
        private float calculateValue(AssetsEntity entity) {
            ...
            try {
                totalValue = parentFragment.getPrices().get(entity.attributes().typeID) * entity.attributes().quantity;
    ...
    public void setScrollPoint(int[] scrollPoint) {
        if (adapter != null) {
            if (absListView instanceof ListView)
                ((ListView) absListView).setSelectionFromTop(scrollPoint[0], scrollPoint[1]);
            else if (absListView instanceof GridView)
                ((GridView) absListView).setSelection(scrollPoint[0]);
        } else {
