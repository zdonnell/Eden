...
public class InventorySort {
    ...
    public static class Count implements Comparator<AssetsEntity> {
        public int compare(AssetsEntity lhs, AssetsEntity rhs) {
            if (lhs instanceof AssetsEntity.Item) {
                ...
                else if (leftItem.attributes().quantity > rightItem.attributes().quantity)
                    return -1;
                else return 0;
            } else {
                ...
                if (leftStation.getContainedAssets().size() < rightStation.getContainedAssets().size())
                    return 1;
                else if (leftStation.getContainedAssets().size() > rightStation.getContainedAssets().size())
                    return -1;
                else return 0;
            }
        }
    ...
    public static class Value implements Comparator<AssetsEntity> {
        ...
        private float calculateValue(AssetsEntity entity) {
            ...
            try {
                totalValue = values.get(entity.attributes().typeID) * entity.attributes().quantity;
