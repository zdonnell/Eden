...
public class StaticTypeDatabase {
	 ...
	 * 
	 * @param typeIDs
	 *            an Array of typeIDs to request information for
	 * @return a {@link SparseArray} of up to date {@link TypeInfo} objects,
	 *         this will not contain values for non stored typeIDs or typeIDs
	 *         whose entry is "out of date." This returned SparseArray should be
	 *         compared against the original ID list to determine which types
	 *         had no information stored.
	 */
	public SparseArray<TypeInfo> getTypeInfo(Integer... typeIDs) {
		...
		for(String[] typeIDStringsSegment : typeChunks) {
			...
			for(int i = 0; i < typeIDStringsSegment.length; i++)
				if(i != typeIDStringsSegment.length - 1)
					whereClause += ",?";
			...
			while(c.moveToNext()) {
				...
				typeInfo.volume = c.getFloat(c.getColumnIndex(TABLE_ROW_M3));
	...
	public void insertTypeInfo(Set<Object> dataSet) {
		TypeInfo[] typeInfoArray = new TypeInfo[dataSet.size()];

		int i = 0;
		for(Object typeInfo : dataSet) {
			typeInfoArray[i] = (TypeInfo) typeInfo;
			i++;
		}
	 ...
	 * 
	 * @param typeInfoSet
	 *            an Array of {@link TypeInfo} objects for insertion
	 */
	public void insertTypeInfo(TypeInfo... typeInfoSet) {
		try {
			...
			for(TypeInfo typeInfo : typeInfoSet) {
				...
				insertValues.put(TABLE_ROW_M3, typeInfo.volume);
	...
	private class CustomSQLiteOpenHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			String newTableQueryString = "create table " + TABLE_NAME + " (" + TABLE_ROW_ID + " integer primary key not null," + TABLE_ROW_GROUPID + " integer," + TABLE_ROW_MARKETGROUPID + " integer," + TABLE_ROW_TYPENAME + " text," + TABLE_ROW_DESCRIPTION + " text," + TABLE_ROW_M3 + " real," + "UNIQUE (" + TABLE_ROW_ID + ") ON CONFLICT REPLACE);";
