...
public class CharactersReqeust extends BaseRequest {
	 ...
	 * 
	 * @param credentials
	 *            the {@link APICredentials} of the account to be queried
	 */
	...
	public ArrayList<EveCharacter> get() {
		...
		POSTData.add(new BasicNameValuePair("keyID", String
				.valueOf(credentials.keyID)));
		POSTData.add(new BasicNameValuePair("vCode",
		...
		/*
		 * Loop through the nodes and build character objects to add to the
		 * return ArrayList
		 */
		for (int x = 0; x < rows.getLength(); x++) {
			...
			String corpName = attributes.getNamedItem("corporationName")
					.getTextContent();
			String charID = attributes.getNamedItem("characterID")
					.getTextContent();
			String corpID = attributes.getNamedItem("corporationID")
					.getTextContent();

			characters.add(new EveCharacter(name, Integer.parseInt(charID),
					corpName, Integer.parseInt(corpID)));
		}

		cachedTime = responseDoc.getElementsByTagName("cachedUntil").item(0)
				.getTextContent();
